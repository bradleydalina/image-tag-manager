...
== Description ==
Image Tag Manager is a WordPress plugin that generates image (alt, title, caption and description) SEO enhancement. It also attempts to fixed filenames before saving, removing unnecessary characters to transform into more meaningful filename and so the image will generate tags based on it.

=== IMPORTANT ===
Image Tag Manager is a free plugin that allows user to automatically generate title tag and alt tags of the images all over your site except the iframed images, with optional filename sanitation and extra settings that allows to add classes and remove srcset and sizes tag.
